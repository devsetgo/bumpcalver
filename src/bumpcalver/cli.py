...
Example:
    ...
    To get a machine-readable result on stdout (e.g. for CI):
        $ bumpcalver --build --json
...
def _compute_new_version(
    *,
    ...
    if beta:
        new_version = apply_prerelease_suffix(
            new_version, config.get("beta_format", ".beta"), current_raw_version
        )
    elif rc:
        new_version = apply_prerelease_suffix(
            new_version, config.get("rc_format", ".rc"), current_raw_version
        )
    elif release:
        new_version = apply_prerelease_suffix(
            new_version, config.get("release_format", ".release"), current_raw_version
        )
    elif custom:
...
@click.option(
    ...
    "json_output",
    is_flag=True,
    help="Emit a single JSON object with the result to stdout instead of "
    "human-readable log lines (those move to stderr). Not compatible with "
...
def main(
    ...
    if json_output and any(undo_options):
        # list_undo_history()/undo_last_operation()/undo_operation_by_id()
        # print their own human-readable output and don't return structured
        # data, so there's nothing for --json to serialize here yet.
        raise click.UsageError("--json cannot be used with --undo, --undo-id, or --list-history.")
    ...
    # Under --json, every informational print() from here on (in this
    # function, and in config.py/handlers.py/backup_utils.py, none of which
    # know about --json) needs to stay off stdout so stdout ends up as
    # nothing but the one JSON payload below. Rather than threading a
    # json_mode flag through every one of those modules, redirect stdout to
    # stderr for the duration and write the final payload directly to the
    # real stdout captured here, before the redirect takes effect.
    ...
    with stdout_redirect:
        ...
        if not file_configs:  # pragma: no cover
            if json_output:
                _emit_json({"error": "No files specified in the configuration."})
            else:
        ...
        def _cached_current_version(file_config: Dict[str, Any]) -> Optional[str]:
            key = (
                file_config["path"],
                file_config.get("variable", ""),
                file_config.get("directive", ""),
            )
        ...
        try:
            ...
            if dry_run:
                if not files_that_would_change:
                    print(
                        f"[dry-run] Version already set to {new_version}; no files would be updated."
                    )
                else:
            ...
            if json_output:
                _emit_json({"error": f"Error generating version: {e}"})
            else:
